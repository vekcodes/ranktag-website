...
export default async function handler(req, res) {
  try {
    ...
    if (id) {
      ...
      if (req.method === 'PUT') {
        const [current] = await sql`SELECT id, status, published_at, publish_at FROM posts WHERE id = ${id}`;
        ...
        const [row] = await sql`
          UPDATE posts SET
            ...
            updated_at = now()
          WHERE id = ${id}
          RETURNING id, slug, status, publish_at`;
        return sendJson(res, 200, { post: row });
      }
    ...
    if (req.method === 'GET') {
      ...
      const rows = await sql`
        SELECT id, slug, title, excerpt, status, tags, author,
               reading_minutes, published_at, publish_at, updated_at, created_at
        FROM posts
        ORDER BY
          CASE status WHEN 'draft' THEN 0 WHEN 'scheduled' THEN 1 ELSE 2 END,
          CASE WHEN status = 'scheduled' THEN publish_at END ASC,
    ...
    if (req.method === 'POST') {
      ...
      const [row] = await sql`
        INSERT INTO posts (
          ...
          reading_minutes, published_at, publish_at
        ) VALUES (
          ...
          ${p.reading_minutes}, ${publishedAt}, ${p.publish_at}
        )
        RETURNING id, slug, status, publish_at`;
